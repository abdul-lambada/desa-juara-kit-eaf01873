<?php
/** @var array $pagination */
/** @var array $filters */
?>
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 font-weight-bold text-primary">Daftar Layanan</h6>
        <a href="/layanan/create" class="btn btn-sm btn-primary">
            <i class="fas fa-plus"></i> Tambah Layanan
        </a>
    </div>
    <div class="card-body">
        <form method="GET" class="row g-2 mb-3">
            <div class="col-sm-4">
                <input type="text" name="q" class="form-control" placeholder="Cari nama layanan" value="<?= htmlspecialchars($filters['q'] ?? '') ?>">
            </div>
            <div class="col-sm-3">
                <select name="aktif" class="form-select">
                    <option value="">Semua Status</option>
                    <option value="1" <?= ($filters['aktif'] ?? '') === '1' ? 'selected' : '' ?>>Aktif</option>
                    <option value="0" <?= ($filters['aktif'] ?? '') === '0' ? 'selected' : '' ?>>Tidak Aktif</option>
                </select>
            </div>
            <div class="col-sm-2">
                <button type="submit" class="btn btn-outline-primary w-100">
                    <i class="fas fa-search"></i> Filter
                </button>
            </div>
        </form>

        <div class="table-responsive">
            <table class="table table-striped table-hover">
                <thead>
                <tr>
                    <th>#</th>
                    <th>Nama</th>
                    <th>Waktu Proses</th>
                    <th>Biaya</th>
                    <th>Status</th>
                    <th>Aksi</th>
                </tr>
                </thead>
                <tbody>
                <?php if (empty($pagination['data'])): ?>
                    <tr>
                        <td colspan="6" class="text-center text-muted">Belum ada data layanan.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($pagination['data'] as $index => $layanan): ?>
                        <tr>
                            <td><?= htmlspecialchars((string) ($pagination['perPage'] * ($pagination['currentPage'] - 1) + $index + 1)) ?></td>
                            <td><?= htmlspecialchars($layanan['nama']) ?></td>
                            <td><?= htmlspecialchars($layanan['waktu_proses'] ?? '-') ?></td>
                            <td><?= $layanan['biaya'] !== null ? 'Rp ' . number_format((float) $layanan['biaya'], 0, ',', '.') : '-' ?></td>
                            <td>
                                <span class="badge bg-<?= !empty($layanan['aktif']) ? 'success' : 'secondary' ?>">
                                    <?= !empty($layanan['aktif']) ? 'Aktif' : 'Tidak Aktif' ?>
                                </span>
                            </td>
                            <td>
                                <a href="/layanan/<?= $layanan['id'] ?>" class="btn btn-sm btn-outline-info"><i class="fas fa-eye"></i></a>
                                <a href="/layanan/<?= $layanan['id'] ?>/edit" class="btn btn-sm btn-outline-warning"><i class="fas fa-edit"></i></a>
                                <form action="/layanan/<?= $layanan['id'] ?>" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus layanan ini?');">
                                    <input type="hidden" name="_method" value="DELETE">
                                    <button type="submit" class="btn btn-sm btn-outline-danger"><i class="fas fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
                </tbody>
            </table>
        </div>

        <?php if (($pagination['lastPage'] ?? 1) > 1): ?>
            <nav>
                <ul class="pagination">
                    <?php for ($i = 1; $i <= $pagination['lastPage']; $i++): ?>
                        <li class="page-item <?= $i === $pagination['currentPage'] ? 'active' : '' ?>">
                            <a class="page-link" href="?page=<?= $i ?>&q=<?= urlencode($filters['q'] ?? '') ?>&aktif=<?= urlencode($filters['aktif'] ?? '') ?>">
                                <?= $i ?>
                            </a>
                        </li>
                    <?php endfor; ?>
                </ul>
            </nav>
        <?php endif; ?>
    </div>
</div>
